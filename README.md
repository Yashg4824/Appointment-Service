...
| **PostgreSQL persistence** | Appointment, reminder, and durable notification-idempotency tables managed by Flyway migrations. The `reminders` table doubles as the work queue. |
| **Duplicate reminder protection** | `UNIQUE (appointment_id, reminder_type)` makes a duplicate reminder row unrepresentable. |
| **Stable notification idempotency** | Each reminder stores the non-null unique key `appointmentId-reminderType`; it is passed to senders and recorded durably before the logging sender emits a notification. |
...
```mermaid
flowchart LR
    W["ReminderWorker<br/>@Scheduled"] <--> DB[("PostgreSQL")]
    W --> NS["NotificationSender<br/>(interface)"]
...
| `scheduler` | `ReminderWorker` — the `@Scheduled` trigger and complete reminder lifecycle: reclaim, claim, process, retry, and outcome updates. |
| `service` | `AppointmentService` (business logic, transaction boundary, and reminder timing), `NotificationSender` interface, and its `LoggingNotificationSender` stub. `ReminderWorker` owns atomic claiming and delivery. |
...
```json
{
  ...
  "reminders": [
    { "id": 5, "idempotencyKey": "5-TWENTY_FOUR_HOURS", "type": "TWENTY_FOUR_HOURS", "scheduledAt": "2026-10-19T18:30:00Z", "status": "PENDING", "attemptCount": 0, "sentAt": null },
    { "id": 6, "idempotencyKey": "5-TWO_HOURS", "type": "TWO_HOURS", "scheduledAt": "2026-10-20T16:30:00Z", "status": "PENDING", "attemptCount": 0, "sentAt": null }
  ]
}
```
...
Reminder times are computed in `AppointmentService` while the appointment and its reminders are
...
### Appointments booked close to their slot

A reminder row is created only if its due time is still in the future:

```text
create the reminder   if   dueAt > now
...
| **Notification sender failure** | The exception is caught by `ReminderWorker`, recorded against that one reminder, and never rethrown. `attempt_count` is incremented, `last_attempt_at` stamped, and the reminder returns to `PENDING` — or goes to `FAILED` once the attempt limit is reached. One bad recipient cannot abandon the rest of the batch. |
...
It is the seam between reminder processing and the outside world. `ReminderWorker` depends on the
...
`ReminderType` is an enum that carries its own `Duration`, and `AppointmentService` iterates
...
failure. `ReminderWorker`'s retry logic depends on nothing else, so any future
...
`ReminderWorker` depends on the `NotificationSender` interface, never on `LoggingNotificationSender`.
...
2. **A reminder whose window has passed is not back-fired**, because the message would be wrong rather
   than merely late.
