 ...
 * does not poll. It also creates the reminder plan as part of the appointment transaction.
 *
 * <p><b>SOLID:</b>
 * <ul>
 ...
 *   <li>Reminder scheduling - which reminders are due and when.</li>
...
public class AppointmentService {
    ...
    public AppointmentResponse create(CreateAppointmentRequest request) {
        ...
        List<Reminder> reminders = reminderPlanFor(appointment.getId(), scheduledAt, now);
    ...
    public AppointmentResponse reschedule(Long appointmentId, Instant newScheduledAt) {
        ...
        for (ReminderType type : ReminderType.values()) {
            ...
            boolean worthSending = dueAt.isAfter(now);
    ...
    private List<Reminder> reminderPlanFor(Long appointmentId, Instant scheduledAt, Instant now) {
        List<Reminder> plan = new ArrayList<>();
        for (ReminderType type : ReminderType.values()) {
            Instant dueAt = type.dueTimeFor(scheduledAt);
            if (dueAt.isAfter(now)) {
                plan.add(new Reminder(appointmentId, type, dueAt, now));
            } else {
                log.info("Skipping {} reminder for appointment {} because due time {} has passed",
                        type, appointmentId, dueAt);
            }
        }
        return plan;
    }
